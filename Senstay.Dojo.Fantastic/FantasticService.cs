...
namespace Senstay.Dojo.Fantastic
{
    public class FantasticService
    {
        ...
        public const string PRICING_AVAILABLE = "available";

        public FantasticService()
        {
        }

        #region Getting Airbnb Listings
        ...
        public CustomStayResult CustomStayListing(int listingId, DateTime startDate, DateTime endDate)
        {
            var parameters = new List<KeyValuePair<string, object>>() {
                                 new KeyValuePair<string, object>("listing_id", listingId.ToString()),
                                 new KeyValuePair<string, object>("start_date", startDate.ToString("yyyy-MM-dd")),
            ...
            var endPoint = RestRequest.GetEndPoint(API_CUSTOMSTAY_SERVICE, parameters);
            return RestRequest.GetCustomStays(endPoint);
        }

        #endregion

        #region Update Airbnb data
        ...
        private PostResponse CustomStay(int listingId, DateTime startDate, DateTime endDate, int minStay)
        {
            // Fantastic API takes request body as the same format as query parameters; not a serialized json object
