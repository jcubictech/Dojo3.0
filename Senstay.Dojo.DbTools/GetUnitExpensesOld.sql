CREATE PROCEDURE [dbo].[GetUnitExpensesOld]
	@StartDate datetime,
	@EndDate datetime,
...
BEGIN
	...
	BEGIN
		...
		WHERE Convert(date, [ExpenseDate]) >= @StartDate and Convert(date, [ExpenseDate]) <= @EndDate and
				[PropertyCode] = @PropertyCode and
				[ExpenseId] = [ParentId] and
				[Category] not like '%Cleaning%' and [Category] not like '%Consum%' and [Category] not like '%Maint%' and [Category] not like '%Laundry%' and
				[IncludeOnStatement] = 1 and
				[IsDeleted] = 0
		ORDER BY [Amount]
	END
