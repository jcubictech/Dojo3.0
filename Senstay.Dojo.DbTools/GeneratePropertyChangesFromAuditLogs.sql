CREATE PROCEDURE [dbo].[GeneratePropertyChangesFromAuditLogs]
...
BEGIN
	...
	BEGIN
		BEGIN TRY
			IF CHARINDEX(';' + @PropertyCode + ';', @LastPropertyCode) <= 0 and 
			   (Not Exists (SELECT 1 FROM [DojoDev].[dbo].[CPL] WHERE [PropertyCode] = @PropertyCode))
			BEGIN
				SET @LastPropertyCode = @LastPropertyCode + ';' + @PropertyCode + ';'
				INSERT INTO #Temp (Line) VALUES('INSERT INTO [dbo].[CPL] ([PropertyCode]) VALUES (''' + @PropertyCode + ''')')
			...
			BEGIN
				IF @NewValue is NULL
					INSERT INTO #Temp (Line) VALUES('UPDATE [dbo].[CPL] SET [' + @ColumnName + '] = NULL WHERE [PropertyCode] = ''' + @PropertyCode + '''')
				ELSE
					INSERT INTO #Temp (Line) VALUES('UPDATE [dbo].[CPL] SET [' + @ColumnName + '] = ''' + REPLACE(LTRIM(RTRIM(@NewValue)), '''', '''''') + ''' WHERE [PropertyCode] = ''' + @PropertyCode + '''')
